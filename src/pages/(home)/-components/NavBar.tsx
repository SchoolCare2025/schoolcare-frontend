...
function NavBar() {
	...
	return (
		...
		>
			...
			<button
				type="button"
				className="z-10 w-6 self-end text-white lg:hidden"
...
function MobileNavigation(props: MobileNavProps) {
	...
	return (
		<section
			className={cnMerge(
				"fixed inset-[0_0_0_auto] overflow-x-hidden bg-school-dark-blue pt-[72px] text-white",
		...
		>
			...
			>
				<NavLinksList
					...
					each={linkItems}
					render={(linkItem) => (
						<NavLink key={linkItem.title} to={linkItem.href} className="[.active]:text-school-blue">
							{linkItem.title}
						</NavLink>
					)}
				/>
