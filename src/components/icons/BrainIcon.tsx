const BrainIcon = (props: React.SVGProps<SVGSVGElement>) => (
	<svg
		width="24"
		height="24"
		viewBox="0 0 24 24"
		fill="none"
		xmlns="http://www.w3.org/2000/svg"
		{...props}
	>
		<path
			d="M7 14C5.34315 14 4 15.3431 4 17C4 18.6569 5.34315 20 7 20C7.35064 20 7.68722 19.9398 8 19.8293"
			stroke="#318CE7"
			strokeWidth="2"
			strokeLinecap="round"
			strokeLinejoin="round"
		/>
		<path
			d="M4.26367 15.6046C2.92405 14.9582 1.99979 13.5871 1.99979 12C1.99979 10.7884 2.53853 9.70253 3.38953 8.96899"
			stroke="#318CE7"
			strokeWidth="2"
			strokeLinecap="round"
			strokeLinejoin="round"
		/>
		<path
			d="M3.42028 8.8882C3.15465 8.49109 2.99976 8.01363 2.99976 7.5C2.99976 6.11929 4.11904 5 5.49976 5C6.06267 5 6.58213 5.18604 7 5.5"
			stroke="#318CE7"
			strokeWidth="2"
			strokeLinecap="round"
			strokeLinejoin="round"
		/>
		<path
			d="M7.23769 5.56533C7.08524 5.24215 7 4.88103 7 4.5C7 3.11929 8.11929 2 9.5 2C10.8807 2 12 3.11929 12 4.5V20"
			stroke="#318CE7"
			strokeWidth="2"
			strokeLinecap="round"
			strokeLinejoin="round"
		/>
		<path
			d="M8 20C8 21.1046 8.89543 22 10 22C11.1046 22 12 21.1046 12 20"
			stroke="#318CE7"
			strokeWidth="2"
			strokeLinecap="round"
			strokeLinejoin="round"
		/>
		<path
			d="M12 7C12 8.65685 13.3431 10 15 10"
			stroke="#318CE7"
			strokeWidth="2"
			strokeLinecap="round"
			strokeLinejoin="round"
		/>
		<path
			d="M20.6094 8.96899C21.4604 9.70253 21.9991 10.7884 21.9991 12C21.9991 12.7031 21.8177 13.3638 21.4992 13.938"
			stroke="#318CE7"
			strokeWidth="2"
			strokeLinecap="round"
			strokeLinejoin="round"
		/>
		<path
			d="M20.5797 8.8882C20.8453 8.49109 21.0002 8.01363 21.0002 7.5C21.0002 6.11929 19.881 5 18.5002 5C17.9373 5 17.4179 5.18604 17 5.5"
			stroke="#318CE7"
			strokeWidth="2"
			strokeLinecap="round"
			strokeLinejoin="round"
		/>
		<path
			d="M12 4.5C12 3.11929 13.1193 2 14.5 2C15.8807 2 17 3.11929 17 4.5C17 4.88103 16.9148 5.24215 16.7623 5.56533"
			stroke="#318CE7"
			strokeWidth="2"
			strokeLinecap="round"
			strokeLinejoin="round"
		/>
		<path
			d="M14 22C12.8954 22 12 21.1046 12 20"
			stroke="#318CE7"
			strokeWidth="2"
			strokeLinecap="round"
			strokeLinejoin="round"
		/>
		<path
			d="M18.6667 16L17 19H21L19.3333 22"
			stroke="#131927"
			strokeWidth="2"
			strokeLinecap="round"
			strokeLinejoin="round"
		/>
	</svg>
);

export default BrainIcon;
